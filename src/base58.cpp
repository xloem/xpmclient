...
inline uint256 Hash(const T1 pbegin, const T1 pend)
{
  ...
  SHA_256 sha;
  sha.init();
  sha.update(pbegin == pend ? pblank : (const unsigned char*)&pbegin[0], (pend - pbegin) * sizeof(pbegin[0]));
  sha.final((unsigned char*)&result);
  sha.init();
  sha.update((const unsigned char*)result.begin(), sizeof(uint256));
  sha.final((unsigned char*)&result);  
  return result;
}
...
bool CBase58Data::SetString(const char* psz, unsigned int nVersionBytes)
{
    ...
    memset(&vchTemp[0], 0, vchData.size());
    return true;
}
